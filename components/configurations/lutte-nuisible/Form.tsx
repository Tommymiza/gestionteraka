...
import { LutteNuisibleItem } from "@/store/lutte-nuisible/type";
...
export default function Formulaire() {
  ...
  const initialValues: Partial<LutteNuisibleItem> = useMemo(
    () => ({
      nom: lutteNuisible?.nom ?? "",
    }),
    [lutteNuisible]
  );

  const handleSubmit = async (values: Partial<LutteNuisibleItem>) => {
    try {
      if (lutteNuisible) {
        ...
        await createLutteNuisible(values);
      }
      cancelEdit();
      getLutteNuisibles();
    } catch (error) {
